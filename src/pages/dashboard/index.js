...
import { httpgetDashboardData, httpgetjobCountbyCategory, httpgetTopEmployer } from '../../services/https';
...
export default function AdminDashboard() {
    ...
    const getData = async () => {
        const dashboardData = await httpgetDashboardData()
        setDashboardStats(dashboardData?.data)
    }
    const getCategoryJobs = async () => {
        const Response = await httpgetjobCountbyCategory()
        setCategoryJobs(Response?.data)
    }
    const getTopEmployer = async () => {
        const Response = await httpgetTopEmployer()
        setTopEmployer(Response?.data)
    ...
    const data = {
        ...
        datasets: [
            {
                label: 'Jobs count',
                data: [categoryJobs?.data?.basic, categoryJobs?.data?.hot, categoryJobs?.data?.premium, categoryJobs?.data?.featured],
                backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#50C878'],
                hoverBackgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#50C878'],
                borderWidth: 1,
            },
        ],
    };
    ...
    return (
        <div className='flex flex-col gap-5'>
            <div className='flex   my-4 h-24 justify-between '>

                <div className='bg-white text-center shadow-md rounded-md  w-56 p-3'>
                    <div className='text-gray-500 font-bold'> Active Employer</div>
                    <div className='text-3xl font-black bg-gradient-to-r from-yellow-400 to-pink-500 bg-clip-text text-transparent'>
                        {dashboardStats?.data?.activeEmployeeCount}
                    </div>
                </div>
                <div className='bg-white shadow-md rounded-md text-center  w-56 p-3'>
                    <div className='text-gray-500 font-bold'> Active Jobs</div>
                    <div className='text-3xl  font-black bg-gradient-to-r from-green-400 to-green-400 bg-clip-text text-transparent'>
                        {dashboardStats?.data?.activeJobCount}

                    </div>
                </div>
                <div className='bg-white shadow-md rounded-md text-center  w-56 p-3'>
                    <div className='text-gray-500 font-bold'> Total Job Engagement</div>

                    <div className='text-3xl font-black bg-gradient-to-r from-blue-700 to-red-200 bg-clip-text text-transparent'>

                        {dashboardStats?.data?.applicantCount}
                    </div>

                </div>
                <div className='bg-white shadow-md rounded-md text-center  w-56 p-3'>
                    <div className='text-gray-500 font-bold'>Pending Employers</div>

                    <div className='text-3xl font-black bg-gradient-to-r from-pink-400 to-red-500 bg-clip-text text-transparent'>

                        {dashboardStats?.data?.nonApprovedEmployerCount}
                    </div>

                </div>
                <div className='bg-white shadow-md rounded-md text-center  w-56 p-3'>
                    <div className='text-gray-500 font-bold'>Pending Jobs</div>

                    <div className='text-3xl font-black bg-gradient-to-r from-yellow-400 to-pink-500 bg-clip-text text-transparent'>

                    {dashboardStats?.data?.nonVerifiedJobCount}
                    </div>
                </div>
            ...
            <div className='flex justify-between gap-6'>
                <div className='w-4/5'>
                    <h2 className='text-lg font-semibold text-gray-500 my-3'>Total jobs by Category </h2>

                    <div className='bg-white shadow-md  rounded-md' style={{ height: '400px' }}>
                ...
                <div className='w-1/3'>
                    <h2 className='text-lg font-semibold text-gray-500 my-3'>Top Employer </h2>

                    <div className='bg-white shadow-md rounded-md  p-5 overflow-auto' style={{ height: '400px' }}>
                        {topEmployer && topEmployer.length !== 0 && topEmployer.map((item, index) => {
                            return (
                                <div className='p-2 my-3 font-semibold text-gray-500 flex gap-2  bg-slate-100 rounded-md shadow-inner'>
                                    <img className='w-12 h-12 object-cover rounded-md' src='https://images.unsplash.com/photo-1575936123452-b67c3203c357?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8aW1hZ2V8ZW58MHx8MHx8&auto=format&fit=crop&w=500&q=60'></img>
                                    <div className='text-xs '>
                                        <h3>{item?.employer?.name}</h3>
                                        <h3>{item?.employer?.companyName}</h3>
                                        <h3> Jobs Posted :{item?.totalJobs}</h3>
                                    </div>
                                </div>
                            )
                        })}
                        ...
                        <div class="chat-message">
                            <div class="flex items-end justify-end">
                                <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-1 items-end">
                                    ...
                                    <div><span class="px-4 py-2 rounded-lg inline-block rounded-br-none bg-blue-600 text-white ">Run this command sudo chown -R `whoami` /Use/.npm-global/ then install the package globally without using sudo</span></div>
                                </div>
                                <img src="https://images.unsplash.com/photo-1590031905470-a1a1feacbb0b?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=3&amp;w=144&amp;h=144" alt="My profile" class="w-6 h-6 rounded-full order-2" />
                            </div>
                        </div>
